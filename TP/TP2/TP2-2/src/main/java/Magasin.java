...
public class Magasin {
	...
	public boolean chercherDichoEtuRec(Article article, int start, int end) {
	    ...
	    if(start < end) {
	        ...
	        if (stock[middle].smallerThan(article)) {
	            return chercherDichoEtuRec(article, middle + 1, end);
	        }
	        else {
	            return chercherDichoEtuRec(article, start, middle - 1);
	        }
	    }
	    else {
	        return false;
	...
	public void afficherStock() {
		...
		for (int i = 0; i < nbArticle; i++) {
			System.out.println(this.getStock()[i].getNumero() + " + " + this.getStock()[i].getPrix());
		}
	}
